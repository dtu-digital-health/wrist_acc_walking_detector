function walking_bouts = post_process_walking_bouts_sliding(acti_counts, stats_w, fs, window_size, resolution, w_post_len, w_p_acf, t_p_acf)
...
%           resolution, time between updates
...
walking_bout_on = find(diff([0; stats_w.walk_b; 0]) == 1) * resolution - resolution + 1;
walking_bout_off = find(diff([0; stats_w.walk_b; 0]) == -1) * resolution - resolution + (window_size-resolution);
