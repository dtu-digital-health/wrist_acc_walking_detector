function [F, WB, WS] = run_walking_detector_sliding(acti_counts, acc, fs, t_acti, resolution)
%RUN_WALKING_DETECTOR_SLIDING returns walking bout predictions.
...
%           resolution, estimate per resolution
%
...
N_windows = floor(length(acti_counts)/(resolution) - (window_size-resolution)/resolution);
stats_w = struct();
for w = 1:N_windows
    % Window indicies
    idx_w = (resolution)*(w - 1) + 1:((resolution)*(w - 1) + window_size); 
...
% Do postprocessing in the sliding version
walking_bouts = post_process_walking_bouts_sliding(acti_counts, stats_w, fs, window_size, resolution, w_post_len, w_p_acf, t_p_acf);
